...
This is a reproduction of an issue I'm having combining react with the google cast sender API.
...
This issue is not entirely consistent, so there's some race condition going on. But I find if I refresh a few times it happens fairly consistently.

*The Setup*
I have a very simple react application created using the [create-react-app](https://reactjs.org/docs/add-react-to-a-new-app.html) tool.

The relevant parts of the App are shown below, but in short, it has a clickable div that triggers an alert, and a 5 second timer to show a second clickable div by changing state:
...
This works as expected. However, when I import the google cast sender API into the HTML page as recommended in the [getting started guide](https://developers.google.com/cast/docs/chrome_sender_integrate#wzxhzdk30wzxhzdk31load_the_library) I start seeing 2 problems on iOS Chrome.

1. Clicking the div's doesn't trigger the alert.
1. When the state change fires, the following error is shown

> TypeError: Argument 1 ('event') to EventTarget.dispatchEvent must be an instance of Event

This error seems to be thrown from deep within the react code:
...
My theory is the same error is being thrown on the click events, but being hidden since it's not triggered by user code, which would explain the non-functioning clicks.

I've done a bit of debugging, and it seems that when I don't import the google cast sender API the `document.createEvent` function returns Events, where as when I'm importing the cast sender API it returns objects.

I've also confirmed that the function is being swapped out somewhere between the initial page load and this call by backing up the `createEvent` function and then comparing it to `document.createEvent` immediately before the `setState` call.

I've also used a timeout to restore the `createElement` function from a backup after it's swapped out. This seems to fix the internal react event, however I then get further failures due to other functions related to Node creation being shimmed in a similar manner.

This is visible with iOS Chrome Version:

    Google Chrome 64.0.3282.112 (Official Build) stable (64-bit)
    Revision  7ceafc6ca46e012a6dbb5933d52fae00eda1f3ca-refs/branch-heads/3282@{#576}
    OS  iOS
    User Agent  Mozilla/5.0 (iPad; CPU OS 11_2_5 like Mac OS X) AppleWebKit/604.1.34 (KHTML, like Gecko) CriOS/64.0.3282.112 Mobile/15D60 Safari/604.1

This behaviour doesn't exhibit on android chrome.

Does anyone have any idea how this could be worked around so I can use both react.js and google cast sending in iOS Chrome?
